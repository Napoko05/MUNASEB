...
var KSearch = function () {
    ...
    $('#btn_drawer_submit').click(function(){
        ...
        switch (opt) {
            case 'pdf':
                ks_load_modal($('#courrier_search_para').data('export-url'), 'modal-xl', formDataToObject(new FormData(form)), '1');
                $('#btn_drawer_cancel').click();
                break;
            case 'excel': 
                let data = new FormData(form);
                let queryParams = new URLSearchParams(data).toString();
                window.location.href = $('#courrier_search_para').data('export-url') + '?' + queryParams;
                $('#btn_drawer_cancel').click();
                break;
            default:
