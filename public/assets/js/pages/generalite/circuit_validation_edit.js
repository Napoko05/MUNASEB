...
var KSMain = function () {
    ...
    return {
        init: function () {
            form = document.querySelector('#frm_new_circuit_adhesion');
            submitButton = document.querySelector('#btn_submit');
            let data = String($('#updatedableField').data('selected') || ''); 
            let selected = data.split(',');
            $('#updatedableField').val(selected);
            $('#frm_new_circuit_adhesion .fselect2').select2();
            handleForm();
        }
    };
}();
