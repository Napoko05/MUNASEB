...
var KSModulemanager = function () {
    ...
    var handleFunctions = () => {
        $('.edit_mod').click(function () {
            ks_load_modal($(table).data('modedit-url'), 'modal-lg', {id: $(this).data('id'), code: $(this).data('code')}, '1');
        });
    };

    var handleStatusModule = () => {
        $('.status_mod').click(function () {
            var id = $(this).data('id');
    ...
    var handleUnintallmod = () => {
        $('.uninstall').click(function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    KTApp.showPageLoading();
                    axios.post($(table).data('uninstall-url'), {id: id})
                            .then(response => {
                                KTApp.hidePageLoading();
                                switch (response.data.status) {
                                    case 'success':
    ...
    return {
        init: function () {
            table = document.querySelector('#dt_module');
        ...
        fct_handleStatusModule(){
            $('.status_fct').click(function () {
            var id = $(this).data('id');
            var statut = $(this).data('statut');
                    ...
                    axios.post($(table).data('statut-url'), {id: id, statut: statut})
                            .then(response => {
                                ...
                                switch (response.data.status) {
                                    case 'success':
                                        notify(lang.msg_save_succes, "success");
                                        $(tr).find('.statut').html($('#new_statut_' + statut).html());
                                        $(tr).find('.status_fct').data('statut', ((statut==1) ? 0:1));
                                        break;
                                    case 'fail':
                                        notify(lang.msg_save_fail, "error");
                                        break;
